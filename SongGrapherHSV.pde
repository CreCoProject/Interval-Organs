...
void setup()
{
  ...
  size(500, 300);
  // Set Color Mode to HSB and Range to 100
  colorMode(HSB, 100);
...
class ColorModel{
  ...
  float delHue; // Delta Value for Hue
  float delSaturation; // Delta Value for Saturation
  float delBrightness; // Delta Value for Brightness
  ...
  ColorModel(){
    ...
    // Choose Hue Delta Value
    delHue = random(100);
    // Choose Saturation Delta Value
    // Switch for Direction
    switch(int(random(2))){
      // Negative Direction
      case 0: delSaturation = -(saturation(c)/6);
      // Positive Direction
      case 1: delSaturation = (100-saturation(c))/6;
    }
    // Choose Brightness Delta Value
    // SWitch for Direction
    switch(int(random(2))){
      // Negative Direction
      case 0: delBrightness = -(brightness(c)/6);
      // Positive Direction
      case 1: delBrightness = (100-brightness(c))/6;
    }
  }
  ...
  color getUpdate(){
    ...
    float hue = hue(c); // Reads Hue Value
    float saturation = saturation(c); // Reads Saturation Value
    float brightness = brightness(c); // Reads Brightness Value 
    
    // Caöculate the Color Transformation
    // Add Delta Values multiple with Centrsl Amount and Map on Color Range
    hue = (hue + (delHue*m))%100; // Calculate for Hue Channel
    saturation = (saturation + (delSaturation*m))%100; // Calculate for Saturation Channel
    brightness = (brightness + (delBrightness*m))%100; // Calculate for Brightness Channel 
  ...
  color offset(ColorModel cm)
  {
    ...
    color cv =  color((hue(ci)+hue(c))%100, (saturation(ci)+saturation(c))/2, (brightness(ci)+brightness(c))/2);
    // Return new Color
    return cv;
  }
